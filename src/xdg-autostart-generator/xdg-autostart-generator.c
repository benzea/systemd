...
static void free_xdgautostartservice(XdgAutostartService *s) {
        if (!s)
                return;
...
DEFINE_TRIVIAL_CLEANUP_FUNC(XdgAutostartService*, free_xdgautostartservice);

static void free_xdgautostartservice_hashmapp(Hashmap **h) {
        hashmap_free_with_destructor(*h, free_xdgautostartservice);
}

static char *xdg_translate_name(const char *name) {
...
static int enumerate_xdg_autostart(Hashmap *all_services) {
        ...
        STRV_FOREACH(path, autostart_dirs) {
                ...
                FOREACH_DIRENT(de, d, log_error_errno(errno, "Failed to enumerate directory %s, ignoring: %m", *path)) {
                        ...
                        _cleanup_(free_xdgautostartservicep) XdgAutostartService *service = NULL;
                        struct stat st;
                        ...
                        name = xdg_translate_name(de->d_name);
                        if (!name)
                                return log_oom();
...
static int format_exec_start(
        ...
        for (i = n = 0; exec_split[i]; i++) {
                ...
                if (r < 0) {
                        log_debug_errno(r, "Failed to unescape '%s': %m", orig);
                        return -EINVAL;
                }
                ...
                if (first_arg) {
                        ...
                        if (r < 0) {
                                log_info_errno(r, "Exec binary '%s' does not exist: %m", c);
                                return r;
                        }
        ...
        *exec_start_out = strv_join(exec_split, " ");

        return 0;
}

static int generate_unit_file(
        ...
        r = format_exec_start(service->exec_string, &exec_start);
        if (r < 0) {
        ...
        unit = prefix_roota(arg_dest, service->name);
        ...
        (void) generator_add_symlink(arg_dest, "xdg-desktop-autostart.target", "wants", service->name);

        return 0;
}

static int run(const char *dest, const char *dest_early, const char *dest_late) {
        _cleanup_(free_xdgautostartservice_hashmapp) Hashmap *all_services = NULL;
        XdgAutostartService *service;
        Iterator j;
        int r;
